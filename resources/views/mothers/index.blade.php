...
<div class="p-8 max-w-7xl mx-auto space-y-8">
    ...
    @else
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="group bg-white rounded-3xl p-8 border border-gray-200 shadow-lg hover:shadow-xl hover:-translate-y-3 transition-all duration-500 overflow-hidden">
                <!-- Foto -->
                <div class="text-center mb-6">
                    @if($mother->foto_url)
                        <img src="{{ $mother->foto_url }}" alt="Foto {{ $mother->nama_lengkap }}" class="w-32 h-32 rounded-full mx-auto object-cover shadow-lg border-4 border-pink-200">
                    @else
                        <div class="w-32 h-32 bg-pink-100 rounded-full mx-auto flex items-center justify-center text-pink-400 text-4xl">
                ...
                <div class="text-center">
                    <h3 class="text-2xl font-bold text-gray-800 mb-2">{{ $mother->nama_lengkap }}</h3>
                    @if($mother->nik)
                        <p class="text-gray-500 text-sm mb-2">NIK: {{ $mother->nik }}</p>
                    @endif
                    <p class="text-gray-500 text-sm mb-2">Minggu Kehamilan: {{ $mother->umur_kehampuan }}</p>
                    <p class="text-gray-500 text-sm mb-4">BB: {{ $mother->berat_badan ? $mother->berat_badan . ' kg' : '-' }}</p>
                    
                    <!-- Status -->
                    <div class="inline-flex items-center px-4 py-2 rounded-full font-bold text-sm mb-6 {{ $mother->status_kesehatan == 'Sehat' || $mother->status_kesehatan == 'Baik' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }}">
                ...
                <div class="flex gap-2 mt-4">
                    <a href="{{ route('mothers.edit', $mother->id) }}" class="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white py-3 px-6 rounded-xl font-bold text-center transition-all flex items-center justify-center space-x-2">
